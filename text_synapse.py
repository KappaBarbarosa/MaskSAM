...
@torch.no_grad()
def test_per_epoch(model, test_dataloader, CM,d_cfg,USE_TEXT,test_config,training, device):
...
def test(data_config, model_config, pretrained_path, batch_size,test_config, device='cuda:0'):
    with torch.no_grad():
        ...
        # testing
        test_IOU,metric_list = test_per_epoch(model, test_dataloader, CM, data_config,USE_TEXT_PROMPT,training,test_config,device)
