...
def read_json(fpath):
    ...
    with open(fpath, 'r') as f:
        obj = json.load(f)
    return obj
def read_image(path,istest=False):
    if(type(path) != str):
        return path
...
class CacheWrapper(TorchDataset):
    ...
    def data_transform(self, image):
        # start = time.time()
        image = cv2.resize(np.array(image), (self.desired_size, self.desired_size), interpolation=cv2.INTER_CUBIC)
...
class DatasetWrapper(TorchDataset):
    ...
    def train_data_transform(self, image):
        image = cv2.resize(image, (self.desired_size, self.desired_size), interpolation=cv2.INTER_CUBIC)
        if np.max(image) > 1:
    ...
    def mask_transform(self, mask):
        if mask.shape[0] != self.desired_size:
