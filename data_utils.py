...
class Organ_Dataset(Dataset):
    def __init__(self, data_path='/home/chen0063/SINICA/SAMed/organ_label_withtext/data.json', data_type='train'):
        super().__init__()
        self.data_path = data_path
        self.type = data_type
        self.table = get_json_data(self.data_path, self.type)

    def __len__(self):
        return len(self.table)
    
    def __getitem__(self, idx):
        image_path, label, GT_path = self.table[idx]
        return image, label, GT_path

    def get_json_data(self, data_path, data_type):
